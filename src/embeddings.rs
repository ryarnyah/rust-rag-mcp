...
/// Every model name the server accepts, paired with the fastembed
/// variant it resolves to. This is the single source of truth for both
/// [`EmbeddingService::new`] and [`EmbeddingService::list_models`], so
/// the `models` command can never advertise less (or more) than `new`
/// actually accepts.
const KNOWN_MODELS: &[(&str, EmbeddingModel)] = &[
    (
...
/// Look a client-supplied model name up in [`KNOWN_MODELS`]. `None`
/// means "not a catalog name" and is what drives [`EmbeddingService::new`]'s
/// warn-and-fallback path.
fn resolve_model(model_name: &str) -> Option<EmbeddingModel> {
    KNOWN_MODELS
        .iter()
...
impl EmbeddingService {
    pub fn new(model_name: &str, cache_dir: &str) -> anyhow::Result<Self> {
        // Unknown names warn and fall back rather than erroring, so a
        // stale `--model` never takes the server down.
        let embedding_model = match resolve_model(model_name) {
            Some(model) => model,
            None => {
...
mod tests {
    ...
    /// The catalog must offer every name `EmbeddingService::new` accepts.
    /// `new` silently falls back to `AllMiniLML6V2` for names it does not
    /// know, so anything missing from `list_models()` is a model the
    /// server can be configured with but the `models` command never
    /// advertises — starting with the CLI's own default.
    #[test]
    fn list_models_contains_the_cli_default_model() {
        let models = EmbeddingService::list_models();
        assert!(
            models.contains(&"Xenova/bge-small-en-v1.5".to_string()),
            "default model missing from catalog: {models:#?}"
        );
    }

    /// The catalog is the contract for `--model`: every listed name must
    /// resolve to a real fastembed variant, never to the unknown-name
    /// fallback.
    #[test]
    fn every_listed_model_resolves_to_itself() {
        for name in EmbeddingService::list_models() {
            assert!(
                resolve_model(&name).is_some(),
                "catalog lists `{name}` but name resolution rejects it"
            );
        }
    }

    /// Names outside the catalog resolve to `None`, which is what makes
    /// [`EmbeddingService::new`] fall back (with a warning) instead of
    /// failing — verified without downloading the fallback model.
    #[test]
    fn unknown_model_names_are_rejected() {
        assert!(resolve_model("definitely/not-a-model").is_none());
        assert!(resolve_model("").is_none());
        assert!(resolve_model("BAAI/bge-small-en-v1.5 ").is_none());
    }
    ...
    fn test_new_valid_model() {
        ...
        assert_eq!(svc.model_name(), "Xenova/bge-small-en-v1.5");
    }
